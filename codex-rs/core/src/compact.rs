...
pub(crate) async fn run_compact_task_on_range(
    ...
    if start == 0 {
         let event = EventMsg::Error(CodexErr::Other("Start index must be greater than 0".to_string()).to_error_event(None));
    ...
    run_compact_task_inner(sess.clone(), turn_context, input, Some((start_idx, end_idx))).await;
}
...
async fn run_compact_task_inner(
    ...
    let (prefix, suffix) = if let Some((start, end)) = range {
        ...
        if start >= all_items.len() || end >= all_items.len() || start > end {
             let event = EventMsg::Error(CodexErr::Other("Invalid history range".to_string()).to_error_event(None));
             sess.send_event(&turn_context, event).await;
             return;
        }
    ...
    let mut new_compacted_slice = build_compacted_history(initial_context, &user_messages, &summary_text);
    ...
    let new_history = if let Some((start, _end)) = range {
         ...
         let mut middle = build_compacted_history(slice_initial_context, &user_messages, &summary_text);
