...
impl ModelClientSession {
    ...
    fn build_responses_plan(&self, prompt: &Prompt) -> Result<ResponsesPlan> {
        ...
        Ok(ResponsesPlan {
            ...
            prompt_cache_key,
            text,
            max_output_tokens: self.state.config.model_max_output_tokens,
            force_store,
            conversation_id,
            previous_response_id,
            caching,
    ...
    fn build_responses_options(
        ...
        ApiResponsesOptions {
            ...
            include: plan.include.clone(),
            prompt_cache_key: plan.prompt_cache_key.clone(),
            text: plan.text.clone(),
            max_output_tokens: plan.max_output_tokens,
            store_override: plan.force_store,
            conversation_id: Some(plan.conversation_id.clone()),
            session_source: Some(self.state.session_source.clone()),
            previous_response_id: plan.previous_response_id.clone(),
            caching: plan.caching.clone(),
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        ...
        let auth_manager = self.state.auth_manager.clone();
        let plan = self.build_responses_plan(prompt)?;
        ...
        loop {
            ...
            let options = self.build_responses_options(&plan, compression);

            let stream_result = client
                .stream_prompt(&self.state.model_info.slug, &plan.api_prompt, options)
                .await;
    ...
    async fn stream_responses_websocket(&mut self, prompt: &Prompt) -> Result<ResponseStream> {
        ...
        loop {
            ...
            let options = self.build_responses_options(&plan, compression);
            let request = self.prepare_websocket_request(&plan.api_prompt, &options);
            ...
            let stream_result = connection
                ...
                .await
                .map_err(map_api_error)?;
            self.websocket_last_items = plan.api_prompt.input.clone();
