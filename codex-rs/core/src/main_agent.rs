...
use codex_protocol::protocol::{InputItem, Submission, Op, ReviewDecision, SubagentType};
...
impl MainAgent {
    ...
    async fn handle_event(&self, event: AgentEvent) -> Result<(), CodexErr> {
        ...
        match event {
            ...
            AgentEvent::SubagentCompleted { subagent_id, agent_type, results } => {
                self.handle_subagent_completed(subagent_id, agent_type, results).await
            }
            AgentEvent::SubagentFailed { subagent_id, agent_type, error, partial_results } => {
                self.handle_subagent_failed(subagent_id, agent_type, error, partial_results).await
            }
            AgentEvent::SubagentCancelled { subagent_id, agent_type, reason } => {
                self.handle_subagent_cancelled(subagent_id, agent_type, reason).await
            }
            AgentEvent::StateTransitionRequested { target_state, reason } => {
                self.handle_state_transition_request(target_state, reason).await
            }
            AgentEvent::SummarizationRequested { context_items, is_final } => {
                self.handle_summarization_request(context_items, is_final).await
            }
        }
    }

    /// Handle user input received event
    async fn handle_user_input_event(&self, input: Vec<InputItem>, sub_id: String) -> Result<(), CodexErr> {
    ...
    async fn handle_subagent_completed(
        ...
        info!("MainAgent handling subagent completion: id={}, type={:?}", subagent_id, agent_type);
        ...
        self.state_manager.handle_subagent_completion(&subagent_id, should_summarize);
        ...
        if should_summarize {
            ...
            self.decide_next_step_after_completion(subagent_id, agent_type, results).await
        }
    }
    ...
    async fn handle_subagent_failed(
        ...
        warn!("MainAgent handling subagent failure: id={}, type={:?}, error={}", 
        ...
        self.handle_failure_recovery(subagent_id, agent_type, error, partial_results).await
    }
    ...
    async fn handle_subagent_cancelled(
        ...
        info!("MainAgent handling subagent cancellation: id={}, type={:?}, reason={}", 
        ...
        self.decide_next_step_after_cancellation(subagent_id, agent_type, reason).await
    }
    ...
    async fn handle_state_transition_request(
        ...
        info!("MainAgent handling state transition request: target={:?}, reason={}", target_state, reason);
    ...
    async fn handle_summarization_request(
        ...
        info!("MainAgent handling summarization request: contexts={}, final={}", 
              context_items.len(), is_final);
    ...
    async fn decide_next_step_for_user_input(&self, input: Vec<InputItem>, sub_id: String) -> Result<(), CodexErr> {
        ...
        info!("MainAgent deciding next step for user input: sub_id={}", sub_id);
    ...
    async fn start_summarization(&self, context_items: Vec<String>) -> Result<(), CodexErr> {
        info!("MainAgent starting summarization with {} context items", context_items.len());
    ...
    async fn decide_next_step_after_completion(
        ...
        info!("MainAgent deciding next step after completion: id={}, type={:?}", subagent_id, agent_type);
    ...
    async fn handle_failure_recovery(
        ...
        warn!("MainAgent handling failure recovery for subagent {}: {}", subagent_id, error);
    ...
    async fn decide_next_step_after_cancellation(
        ...
        info!("MainAgent deciding next step after cancellation: id={}, reason={}", subagent_id, reason);
    ...
    /// Perform summarization
    async fn perform_summarization(&self, context_items: Vec<String>, is_final: bool) -> Result<(), CodexErr> {
        info!("MainAgent performing summarization: contexts={}, final={}", context_items.len(), is_final);
    ...
    fn handle_submission(&self, submission: Submission) -> Result<AgentTask, anyhow::Error> {
        match submission.op {
            Op::UserInput { items } => {
                ...
                Err(anyhow::anyhow!("UserInput converted to event - processing in MainAgent event loop"))
            }
            Op::UserTurn { items, .. } => {
                // Convert to event and send to the event loop
                let event = AgentEvent::user_input_received(items, submission.id.clone());
                ...
                // Return a placeholder task - the actual processing happens in the event loop
                Err(anyhow::anyhow!("UserTurn converted to event - processing in MainAgent event loop"))
            }
            _ => {
