...
async fn compact_range_middle() {
    ...
    let sse1 = sse(vec![ev_assistant_message("m1", "reply1"), ev_completed("r1")]);
    let sse2 = sse(vec![ev_assistant_message("m2", "reply2"), ev_completed("r2")]);
    let sse3 = sse(vec![ev_assistant_message("m3", "reply3"), ev_completed("r3")]);

    // Mock response for compaction (summarizing msg2 and reply2)
    let summary_text = "Summary of msg2 and reply2";
    let sse_compact = sse(vec![ev_assistant_message("m_summary", summary_text), ev_completed("r_compact")]);
    ...
    let NewConversation { conversation: codex, .. } = conversation_manager.new_conversation(config).await.unwrap();

    // 1. Submit "msg1"
    codex.submit(Op::UserInput { items: vec![UserInput::Text { text: "msg1".into() }] }).await.unwrap();
    wait_for_event(&codex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;

    // 2. Submit "msg2"
    codex.submit(Op::UserInput { items: vec![UserInput::Text { text: "msg2".into() }] }).await.unwrap();
    wait_for_event(&codex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;

    // 3. Submit "msg3"
    codex.submit(Op::UserInput { items: vec![UserInput::Text { text: "msg3".into() }] }).await.unwrap();
    wait_for_event(&codex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    ...
    codex.submit(Op::ManageHistory { action: HistoryAction::Compact { start: 5, end: 6 } }).await.unwrap();
    wait_for_event(&codex, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    ...
    let has_msg2 = input.iter().any(|item| item["content"][0]["text"].as_str() == Some(msg2_text));
    let has_reply2 = input.iter().any(|item| item["content"][0]["text"].as_str() == Some(reply2_text));
    let has_msg1 = input.iter().any(|item| item["content"][0]["text"].as_str() == Some("msg1"));
    ...
    codex.submit(Op::ManageHistory { action: HistoryAction::ViewAll }).await.unwrap();
    let view_event = wait_for_event(&codex, |ev| matches!(ev, EventMsg::HistoryView(_))).await;
    let EventMsg::HistoryView(view) = view_event else { panic!("expected history view") };
