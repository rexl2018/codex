        ...
        match event {
            ...
            AppEvent::ResumeSession(path) => {
                ...
                let init = crate::chatwidget::ChatWidgetInit {
                    ...
                    is_first_run: false,
                    feedback_audience: self.feedback_audience,
                    model: Some(resumed.session_configured.model.clone()),
                    otel_manager: self.otel_manager.clone(),
                };
            ...
            AppEvent::AddWritableDir(path) => {
                ...
                match path.canonicalize() {
                    Ok(path) => {
                        ...
                        let current_policy = self.config.sandbox_policy.get().clone();
                        match current_policy {
                            codex_core::protocol::SandboxPolicy::WorkspaceWrite {
                                ...
                                match codex_utils_absolute_path::AbsolutePathBuf::try_from(
                                    path.clone(),
                                ) {
                                    Ok(abs_path) => {
                                        if !writable_roots.contains(&abs_path) {
                                            writable_roots.push(abs_path);
                                            if let Err(err) = self.config.sandbox_policy.set(
                                                codex_core::protocol::SandboxPolicy::WorkspaceWrite {
                                                    writable_roots,
                                                    network_access,
                                                    exclude_tmpdir_env_var,
                                                ...
                                                self.chat_widget.add_error_message(format!(
                                                    "Failed to update sandbox roots: {err}"
                                                ));
                                            } else {
                                                let _ = self.chat_widget.set_sandbox_policy(
                                                    self.config.sandbox_policy.get().clone(),
                                                );
                                            }
                                        }
                                    }
...
mod tests {
    ...
    #[tokio::test]
    async fn copy_last_agent_message_writes_to_file_when_filename_provided() {
        let mut app = make_test_app().await;
