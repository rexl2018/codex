        ...
        match key_event {
            ...
            KeyEvent {
                ...
                if let Some(sel) = popup.selected_item() {
                    match sel {
                        CommandItem::Builtin(cmd) => {
                            // Check if there are arguments after the command name
                            if let Some((_, rest, _)) = parse_slash_name(first_line)
                                && !rest.is_empty()
                                && matches!(
                                    cmd,
                                    SlashCommand::Review | SlashCommand::Hist | SlashCommand::Chat
                                )
                            {
                                self.textarea.set_text_clearing_elements("");
...
mod tests {
    ...
    fn set_text_content_places_cursor_at_end() {
        ...
        composer.set_text_content("some draft".to_string(), Vec::new(), Vec::new());
    ...
    fn history_navigation_continues_from_end_cursor() {
        ...
        composer
            .history
            .record_local_submission(HistoryEntry::from_text("first entry".to_string()));
        composer
            .history
            .record_local_submission(HistoryEntry::from_text("second entry".to_string()));
...
程序员 与 Unicode 同行";
    ...
    /// Behavior: `/hist ll` should dispatch as CommandWithArgs(Hist, "ll") when the popup
    /// is NOT visible (cursor moved past the command name into the args area).
    #[test]
    fn hist_command_with_args_no_popup() {
        use crossterm::event::KeyCode;
        use crossterm::event::KeyEvent;
        use crossterm::event::KeyModifiers;
        ...
        // Type "/hist ll" character by character to simulate user input.
        // This will cause the popup to appear initially for "/hist", but when
        // we move cursor past "hist" into the args area, the popup will close.
        type_chars_humanlike(&mut composer, &['/', 'h', 'i', 's', 't', ' ', 'l', 'l']);

        // Verify the text is correct
        assert_eq!(composer.textarea.text(), "/hist ll");

        // Now press Enter to dispatch the command
        let (result, _needs_redraw) =
            composer.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));

        match result {
            InputResult::CommandWithArgs(cmd, args) => {
                assert_eq!(cmd, SlashCommand::Hist, "expected SlashCommand::Hist");
                assert_eq!(args, "ll", "expected args to be 'll'");
            }
            InputResult::Command(cmd) => {
                panic!(
                    "expected CommandWithArgs but got Command({:?}) - args were lost!",
                    cmd.command()
                )
            }
            InputResult::Submitted { text, .. } => {
                panic!(
                    "expected CommandWithArgs but got Submitted with text: {text} - command not recognized!"
                )
            }
            InputResult::Queued { .. } => {
                panic!("expected CommandWithArgs but got Queued")
            }
            InputResult::None => {
                panic!("expected CommandWithArgs but got None")
            }
        }
        assert!(
            composer.textarea.is_empty(),
            "composer should be cleared after command dispatch"
        );
    }
